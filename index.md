---
title: Início
nav_order: 1
permalink: /
---

# Manual do RIT360 Realiza

O **RIT360 Realiza** reúne num lugar só o trabalho que hoje se perde entre
conversas: quem faz o quê, para quando, o que já foi entregue e o que foi
aprovado — com o registro de quem aprovou e quando.

Ele foi feito para organizações da sociedade civil. Isso muda o produto de
verdade: **não é preciso aprender um método antes de começar a usar.** Você cria
um espaço, escreve as tarefas e trabalha. Os conceitos mais avançados só
aparecem no dia em que resolvem um problema seu — e, se você nunca tiver esse
problema, eles nunca aparecem.

{: .important }
**Vai usar pelo celular?** Instale o Realiza na tela inicial: ele abre como aplicativo e avisa
você mesmo com o celular bloqueado. Leva menos de um minuto — veja
[Instalar no celular](/instalar-no-celular/).

## Comece por aqui

Se é a sua primeira vez, leia [Primeiros passos](/primeiros-passos/): entrar,
aceitar um convite, recuperar a senha e entender em que organização você está.

Depois, vá direto ao guia do seu papel. Ele é uma página curta, com os links do
que você precisa saber e nada mais:

| Se você… | Leia |
|---|---|
| cuida da organização inteira, dos membros e das configurações | [Quem administra a organização](/papeis/administrador-da-organizacao/) |
| toca um projeto ou uma operação, e organiza o trabalho dos outros | [Quem gerencia um espaço](/papeis/gestor-de-espaco/) |
| executa as tarefas | [Quem executa o trabalho](/papeis/executor/) |
| aprova o material produzido | [Quem aprova](/papeis/aprovador/) |
| é de fora e foi convidado para acompanhar ou aprovar | [Quem é convidado de fora](/papeis/convidado/) |
| cuida da instalação do Realiza | [Quem administra o serviço](/papeis/administrador-do-servico/) |

## O caminho completo, do começo ao fim

Estes são os passos de um trabalho que nasce, é feito, é aprovado e se encerra.
Você não precisa de todos — a maior parte das organizações usa os quatro
primeiros e nunca chega perto dos outros.

1. [Criar um espaço](/trabalho/criar-espaco/) — de projeto ou de operação contínua
2. [Chamar pessoas](/trabalho/chamar-pessoas/) — de dentro e de fora
3. [Criar e organizar tarefas](/trabalho/criar-tarefas/)
4. [Ver o trabalho](/trabalho/visoes/) — em quadro, lista ou calendário
5. [Anexar material](/trabalho/anexar-material/) a uma tarefa
6. [Enviar para aprovação](/trabalho/enviar-para-aprovacao/)
7. [Aprovar ou pedir ajuste](/trabalho/aprovar-ou-pedir-ajuste/)
8. [Corrigir e reenviar](/trabalho/corrigir-e-reenviar/)
9. [Ver o que está travado](/trabalho/meu-trabalho/) — a primeira tela do dia
10. [Arquivar e reabrir](/trabalho/arquivar-e-reabrir/)

## Outras portas de entrada

- [Assistente](/assistente/) — pergunte o que precisa da sua atenção, ou peça para criar uma
  tarefa, escrevendo em linguagem comum, dentro do app ou pelo [Telegram](/configuracoes/telegram/)
- [Conhecimento](/conhecimento/) — documentos, links e páginas de wiki que a organização guarda
  para o time, com histórico de versões e a opção de publicar uma página para quem não tem conta

## Quando travar

- [Perguntas frequentes](/faq/) — o que as pessoas mais perguntam
- [Glossário](/glossario/) — o que cada palavra do produto quer dizer
- [Enviar feedback](/enviar-feedback/) — achou um problema, ou faltou alguma coisa
- [Novidades](/novidades/) — o que mudou no produto, e quando
- [O que ainda vem por aí](/em-breve/) — o que o Realiza ainda não faz
- [Termos de Uso](/termos/) e [Política de Privacidade](/privacidade/) — quem responde pelos seus dados, o que acontece com eles e o que muda quando a organização usa inteligência artificial

{: .note }
Este manual descreve o que o produto faz **hoje**. Encontrando diferença entre
o que está escrito aqui e o que você vê na tela, a tela é que está certa — e
nos avise pelo [feedback](/enviar-feedback/), porque manual desatualizado é
problema nosso, não seu.
